...
class OpenAIProviderService:
    ...
    def list_models(self, api_key: str | None = None) -> list[dict[str, Any]]:
        ...
        return [
            {"id": model.id, "object": getattr(model, "object", None)}
            for model in models.data
        ]

    def generate_completion(self, api_key: str | None = None, prompt: str = "", model: str = "gpt-4o-mini") -> dict[str, Any]:
        client = self._get_client(api_key)
        completion = client.responses.create(model=model, input=prompt)
        return {"id": completion.id, "text": completion.output_text}
